"""Command line: ``python -m agentgate <command>``. Exit codes: 0 ok, 1 configuration or provider error, 2 broken ledger."""
...
from agentgate.context import build_context
from agentgate.demo import SCENARIOS
from agentgate.enrichment import EnrichmentStore, FakeEnricher
from agentgate.money import rupees
from agentgate.payments import PaymentsError
from agentgate.policy import PolicyError
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="agentgate",
                                description="AgentGate: makes a Razorpay merchant sellable to AI buyer agents, safely.")
    p.add_argument("--db", help="SQLite file (default: AGENTGATE_DB or agentgate.db)")
...
def _dispatch(args, settings: Settings) -> int:
    ...
    if cmd == "seed":
        ...
        from agentgate.catalog import seed
        ...
        if args.push:
            from agentgate.razorpay_client import push_items
    ...
    if cmd == "sync-items":
        _require_razorpay(settings)
        from agentgate.razorpay_client import sync_items
    ...
    if cmd == "serve":
        ...
        print(f"AgentGate for {settings.merchant_name}: payments={ctx.payments_mode}, "
    ...
    if cmd == "metrics":
        from agentgate.metrics import main as metrics_main
    ...
    if cmd == "demo":
        from agentgate.demo import run_demo
