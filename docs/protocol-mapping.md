...
As of 3 September 2026. Nothing here claims conformance; it says which vocabulary and which design AgentGate
...
for buyer-facing errors and a standard error shape. AgentGate's agent API has the same shape and the same
...
| `POST /checkout_sessions/{id}/complete` | same | ACP charges a delegated payment token synchronously; AgentGate returns a Razorpay Payment Link and the status `payment_pending` |
...
AP2 chains signed mandates: intent, cart and payment. AgentGate borrows the *mandate* vocabulary for the
...
authorization and MCP for tools. AgentGate's discovery document and feed play the discovery role; an MCP server
...
and audit trails. AgentGate models the merchant's side of that picture: a registry of agents with revocation
