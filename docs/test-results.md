...
Generated 2026-09-03 18:31 UTC by `python scripts/make_test_report.py` (version 0.1.0) from a full `pytest` run on this machine. Nothing below is typed by hand.

## Summary

| Metric | Value |
|---|---|
| Test cases collected | 282 |
| Passed | 282 / 282 (100.0%) |
| Failed | 0 |
| Skipped | 0 |
| Test files | 22 |
| Total test time | 9.4 s |
| Mean per test | 33 ms |
...
| test_sessions.py | sessions | 38 | 38 / 38 (100%) | 0.51 s | state machine: create, update, idempotency, complete, reserve, retry, abandon, cancel, review queue, refunds, spend across sessions, trail |
| test_api.py | api | 20 | 20 / 20 (100%) | 1.26 s | discovery, feed, auth, idempotency, error shape, full flow, webhook signature and dedupe |
| test_policy.py | policy | 17 | 17 / 17 (100%) | 0.04 s | merchant policy validation including review threshold and refund window |
| test_buyer.py | buyer | 15 | 15 / 15 (100%) | 0.82 s | client errors, scripted runs for every scenario, LLM planner loop, replanning, parallel tool calls, fail-closed behaviour |
| test_enrichment.py | enrichment | 15 | 15 / 15 (100%) | 0.05 s | proposal validation, keyword and LLM enrichers, pending / approve / reject |
| test_cli.py | cli | 13 | 13 / 13 (100%) | 3.07 s | every command on a temporary database, exit codes, review, refund, eval, replay |
| test_dashboard.py | dashboard | 11 | 11 / 11 (100%) | 1.15 s | login, products and enrichment approval, agents, policy, sessions, review, refund, ledger verify, replay, receipt |
| test_demo.py | demo | 11 | 11 / 11 (100%) | 0.78 s | all seven scenarios end to end on fakes, policy restored after the review scenario |
...
| test_ledger.py | ledger | 9 | 9 / 9 (100%) | 0.04 s | hash chain, verify, tamper detection, receipt rendering, filtering |
| test_crosssell.py | cross-sell | 8 | 8 / 8 (100%) | 0.00 s | headroom under every cap, candidate filtering, fake and LLM pickers |
| test_catalog.py | catalog | 7 | 7 / 7 (100%) | 0.02 s | seed data, raw seed, feed visibility of approved fields, search, snapshot |
...
| test_agents.py | agents | 6 | 6 / 6 (100%) | 0.02 s | key issuance, hashing at rest, authentication, revocation |
...
| test_metrics.py | metrics | 5 | 5 / 5 (100%) | 1.39 s | batch invariants: overruns zero, denials explained, determinism per seed |
| test_replay.py | replay | 4 | 4 / 4 (100%) | 0.10 s | recorded decisions replay identically; changed inputs are reported |
...
| Layer | Cases | Share |
|---|---|---|
| Unit: pure functions and stores (gate, ledger, mandates, catalog, policy, agents, llm parsing, eval, replay) | 158 | 56% |
| Service: state machine, adapters, buyer loop, batch | 69 | 24% |
| Surface: HTTP API, dashboard, CLI, demo scenarios | 55 | 20% |
...
| Test | Time |
|---|---|
| test_metrics.py::test_run_batch_is_deterministic_for_a_seed | 0.63 s |
| test_cli.py::test_refund_command | 0.48 s |
| test_metrics.py::test_batch_exercises_every_path | 0.41 s |
| test_cli.py::test_review_commands | 0.37 s |
| test_cli.py::test_ledger_verify_show_tamper_receipt | 0.36 s |
| test_cli.py::test_demo_refused_and_happy_on_fake_payments | 0.33 s |
| test_cli.py::test_enrich_approve_reject | 0.29 s |
| test_api.py::test_well_known_needs_no_auth | 0.27 s |
| test_cli.py::test_ledger_replay_command | 0.27 s |
| test_metrics.py::test_run_batch_counts_and_invariants | 0.22 s |
