...
The pitch: "AgentGate makes a Razorpay merchant sellable to AI agents, safely."
...
Single Python package `agentgate/`, one FastAPI process, one SQLite file. Money is integer paise everywhere.
...
| Buyer agent (demo client) | `buyer/` | the public API only | LLM endpoint, AgentGate API | outside the merchant trust boundary |
| Metrics | `metrics.py` | scripted runs on fakes | — | honest numbers |
| CLI | `cli.py` | — | everything | `python -m agentgate ...` |
