...
python -m agentgate seed --raw --push      # --push needs test keys; drop it to stay offline
python -m agentgate serve                  # leave this running; note the dashboard login URL it prints
```
...
let a machine buy without letting it go rogue? AgentGate is the merchant's answer, on Razorpay test mode.
...
- Click **Propose enrichment**. Show one proposal side by side with the raw text: category, tags, attributes,
  ...
  ignored it. Approve the proposals (or `python -m agentgate approve --all`).
...
```powershell
python -m agentgate demo --scenario crosssell --planner llm --payments real
```

Narrate the agent output: it browses, creates a session, the gate returns ALLOW with the 14-check trail, the
...
```powershell
python -m agentgate demo --scenario replan --planner llm --payments fake
```
...
```powershell
python -m agentgate demo --scenario payfail --payments real
```
