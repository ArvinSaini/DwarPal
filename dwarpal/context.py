...
from dwarpal.catalog import Catalog
...
def build_context(settings: Settings, *, conn=None, clock: Callable[[], int] | None = None, payments=None,
    ...
    if payments is None:
        if settings.razorpay_configured and not use_fake_payments:
            from dwarpal.razorpay_client import RazorpayPayments  # the only place the SDK is imported
    ...
    if picker is None or enricher is None:
        ...
        if picker is None:
            if llm is not None:
                ...
                picker = FakePicker()
        if enricher is None:
            from dwarpal.enrichment import FakeEnricher, LLMEnricher
